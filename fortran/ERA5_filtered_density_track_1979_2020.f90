...
! The original code looks like it was made to compute all ERA5 storm tracks, 
! but will only use the filtered tracks for this version.
...
!parameter (linemax = 1833513, lineheader = 16, nummax = 24604, topnum= 30, px=800)
...
! Output data
  R=2.5e5 !in meters
  fileout='/pampa/cloutier/Track_filtered_density_1979_2020_all_seasons_250km.nc'

! Define a virtual domain covers the entire domain of the filtered ETCs

! Lon: 0 to 359.5 (vix=359.5/0.5+1)
! Lat: 0 to 90  (vjx=90/0.5+1)
...
! input data
open (10, FILE='/pampa/cloutier/ERA5_stormtrack_filtered.txt', STATUS='old')
